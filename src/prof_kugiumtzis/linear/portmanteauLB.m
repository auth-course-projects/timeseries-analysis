...
if ~isempty(tittxt)
    ...
    plot([1:maxtau]', pV, '.-k', 'linewidth', 1.5)
    plot([0 maxtau+1],alpha*[1 1], '--c', 'linewidth', 1.5)   
    xlabel('lag (\tau)'), ylabel('p-value')
    title([tittxt ' Ljung-Box Portmanteau test'], 'FontName', 'JetBrains Mono', 'FontSize', 14)
    axis([0 maxtau+1 0 1])
    set(gca, 'FontName', 'JetBrains Mono')
